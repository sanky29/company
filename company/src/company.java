	...
	//define recursive function for ith level employee 
	public static void print_employees_ith(employee x, int i) {
		
		//if i = level + 1 then print all employee of x
		if (x.show_level() == i - 1 ) {
		     x.print_employee() ;
		}
		//else if they are still at low level dont do anything
		else if (x.show_level() >= i) {
			
		}
		//else call ith print function for every under employee
		else {
			if (x.show_no() != 0) {
	...
	//now for all level employee call print ith employee from 1 to max_level
	//x = ceo here
